...
let matchedData: Record<string,any>[];
// Sorted data
let sortedData: Record<string,any>[] = [];
// Data of the current page 
let displayedData: Record<string,any>[] = [];
...
let filterObj: Record<string,Record<string,any>> = {};
...
function getRegexGroups(phrase: string) {
...
function applyFilters() {
    ...
    if (searchObj.value) {
        let rgx: RegExpExecArray|null = getRegexGroups(searchObj.value);
        let regexp: RegExp;
        if (rgx) {
            try {
    ...
    Object.entries(filterObj).forEach(([key, filter]) => {
        let rgx: RegExpExecArray|null = null;
